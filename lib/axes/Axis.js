...
function tickHelper(props, scale) {
	...
	if ((0, _utils.isDefined)(tickValuesProp)) {
		...
		if (props.isDetailMobile) {
			var _scale$domain = scale.domain(),
		    _scale$domain2 = _slicedToArray(_scale$domain, 2),
		    min = _scale$domain2[0],
		    max = _scale$domain2[1];
			var tempArray = []
			const step = (max - min) / (tickArguments + 1)
			for (let index = 0; index < tickArguments; index++) {
				tempArray.push(Math.round(min + (index + 1)*step))
			}
		...
		if (scaleType === "log") {
			if (tickValues.length >= 20) {
				tickValues = tickValues.filter(function (x) {
					return isFiveBase(x);
				});
			}
