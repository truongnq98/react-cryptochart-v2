...
function _drawOnCanvas(ctx, props, moreProps) {
	...
	var showHighLow = props.showHighLow;
	var dFormat = props.dFormat;
	var textColor = props.textColor;
	var paddingChart = props.paddingChart;

	var highest = 0,
		lowest = 0;

	// const wickData = getWickData(props, xAccessor, xScale, yScale, plotData);
	plotData.forEach(function (x, index) {
		if (x.high >= plotData[highest].high) {
			highest = index;
		}
		if (x.low < plotData[lowest].low) {
			lowest = index;
		}
	})
	...
	if (showHighLow) {
		...
		ctx.font = "6px Arial";
		ctx.fillText('▲', candleData[lowest].wick.x - 3, candleData[lowest].wick.y4 + paddingChart);
		ctx.fillText('▼', candleData[highest].wick.x - 3, candleData[highest].wick.y1 - paddingChart + 3);
		if (fontSize) {
			ctx.font = fontSize + "px Arial";
		} else {
			ctx.font = "12px Arial";
		}
		var indexLowest = candleData[lowest].wick.x;
		var indexHighest = candleData[highest].wick.x;
		if (dFormat) {
			if ((indexLowest + 5 + ctx.measureText(dFormat(plotData[lowest].low)).width) > candleData[candleData.length - 1].wick.x) {
				indexLowest -= ctx.measureText(dFormat(plotData[lowest].low)).width + 5;
			} else {
				indexLowest += 5;
			}
			if ((indexHighest + 5 + ctx.measureText(dFormat(plotData[highest].high)).width) > candleData[candleData.length - 1].wick.x) {
				indexHighest -= ctx.measureText(dFormat(plotData[highest].high)).width + 5;
			} else {
				indexHighest += 5;
			}
			ctx.fillText(dFormat(plotData[lowest].low), indexLowest, candleData[lowest].wick.y4 + paddingChart);
			ctx.fillText(dFormat(plotData[highest].high), indexHighest, candleData[highest].wick.y1 - paddingChart + 3);
		} else {
			if ((indexLowest + 5 + ctx.measureText(plotData[lowest].low).width) > candleData[candleData.length - 1].wick.x) {
				indexLowest -= ctx.measureText(plotData[lowest].low).width + 5;
			} else {
				indexLowest += 5;
			}
			if ((indexHighest + 5 + ctx.measureText(plotData[highest].high).width) > candleData[candleData.length - 1].wick.x) {
				indexHighest -= ctx.measureText(plotData[highest].high).width + 5;
			} else {
				indexHighest += 5;
			}
			ctx.fillText(plotData[lowest].low, indexLowest, candleData[lowest].wick.y4 + paddingChart);
			ctx.fillText(plotData[highest].high, indexHighest, candleData[highest].wick.y1 - paddingChart + 3);
		}
	}
