...
function _drawOnCanvas(ctx, props, moreProps) {
	...
	if (showHighLow) {
		...
		ctx.fillText('▼', candleData[highest].wick.x - 3, candleData[highest].wick.y1 - paddingChart);
		if (fontSize) {
		...
		if (dFormat) {
			if ((indexLowest + 10 + ctx.measureText(dFormat(plotData[lowest].low)).width) > candleData[candleData.length - 1].wick.x) {
				indexLowest -= ctx.measureText(dFormat(plotData[lowest].low)).width + 10;
			} else {
				indexLowest += 10;
			}
			if ((indexHighest + 10 + ctx.measureText(dFormat(plotData[lowest].high)).width) > candleData[candleData.length - 1].wick.x) {
				indexHighest -= ctx.measureText(dFormat(plotData[lowest].high)).width + 10;
			} else {
				indexHighest += 10;
			}
			ctx.fillText(dFormat(plotData[lowest].low), indexLowest, candleData[lowest].wick.y4 + paddingChart);
			ctx.fillText(dFormat(plotData[highest].high), indexHighest, candleData[highest].wick.y1 - paddingChart);
		} else {
			if ((indexLowest + 10 + ctx.measureText(plotData[lowest].low).width) > candleData[candleData.length - 1].wick.x) {
				indexLowest -= ctx.measureText(plotData[lowest].low).width + 10;
			} else {
				indexLowest += 10;
			}
			if ((indexHighest + 10 + ctx.measureText(closeHighest.high).width) > candleData[candleData.length - 1].wick.x) {
				indexHighest -= ctx.measureText(closeHighest.high).width + 10;
			} else {
				indexHighest += 10;
			}
			ctx.fillText(plotData[lowest].low, indexLowest, candleData[lowest].wick.y4 + paddingChart);
			ctx.fillText(plotData[highest].high, indexHighest, candleData[highest].wick.y1 - paddingChart);
		}
	}
