...
function _drawOnCanvas(ctx, props, moreProps) {
	...
	if (showHighLow) {
		...
		if (dFormat) {
			...
			if ((indexHighest + 5 + ctx.measureText(dFormat(plotData[lowest].high)).width) > candleData[candleData.length - 1].wick.x) {
				indexHighest -= ctx.measureText(dFormat(plotData[lowest].high)).width + 5;
			} else {
				indexHighest += 5;
			}
			...
			if ((indexHighest + 5 + ctx.measureText(closeHighest.high).width) > candleData[candleData.length - 1].wick.x) {
				indexHighest -= ctx.measureText(closeHighest.high).width + 5;
			} else {
				indexHighest += 5;
			}
