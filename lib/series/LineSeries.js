...
​
			if (showHighLow) {
				...
				ctx.fillText('▼', Math.round(xScale(xAccessor(closeHighest))) - 3, Math.round(yScale(yAccessor(closeHighest))) - paddingChart);
				if (fontSize) {
				...
				if (dFormat) {
					if ((indexLowest + 10 + ctx.measureText(dFormat(closeLowest.close)).width) > Math.round(xScale(xAccessor(plotData[plotData.length - 1])))) {
						indexLowest -= ctx.measureText(dFormat(closeLowest.close)).width + 10;
					} else {
						indexLowest += 10;
					}
					if ((indexHighest + 10 + ctx.measureText(dFormat(closeHighest.close)).width) > Math.round(xScale(xAccessor(plotData[plotData.length - 1])))) {
						indexHighest -= ctx.measureText(dFormat(closeHighest.close)).width + 10;
					} else {
						indexHighest += 10;
					}
					ctx.fillText(dFormat(closeLowest.close), indexLowest, Math.round(yScale(yAccessor(closeLowest))) + paddingChart);
					ctx.fillText(dFormat(closeHighest.close), indexHighest, Math.round(yScale(yAccessor(closeHighest))) - paddingChart);
				} else {
					if ((indexLowest + 10 + ctx.measureText(closeLowest.close).width) > Math.round(xScale(xAccessor(plotData[plotData.length - 1])))) {
						indexLowest -= ctx.measureText(closeLowest.close).width + 10;
					} else {
						indexLowest += 10;
					}
					if ((indexHighest + 10 + ctx.measureText(closeHighest.close).width) > Math.round(xScale(xAccessor(plotData[plotData.length - 1])))) {
						indexHighest -= ctx.measureText(closeHighest.close).width + 10;
					} else {
						indexHighest += 10;
					}
					ctx.fillText(closeLowest.close, indexLowest, Math.round(yScale(yAccessor(closeLowest))) + paddingChart);
					ctx.fillText(closeHighest.close, indexHighest, Math.round(yScale(yAccessor(closeHighest))) - paddingChart);
				}
			}
​
			ctx.stroke();
